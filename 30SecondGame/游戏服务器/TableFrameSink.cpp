...
#define IDI_RESET_STORAGE           4                                   //清空库存
			...
			if (m_wCurrentBanker!=wChairID)
			{
				...
				if (m_pITableFrame->GetGameStatus() != GAME_SCENE_END)
				{
					...
					if (lExitScore!=0L) 
					{
						...
						TCHAR szMsg[128]={0,};
						_sntprintf(szMsg,CountArray(szMsg),TEXT("闲家 %s强退！"),pIServerUserItem->GetNickName());
					...
					case KEY_STOCK:
						{
							...
							strMsg.Format(TEXT("库存剩余量：%I64d"), m_StorageControl.GetStroageScore());
						}
						break;
...
bool CTableFrameSink::OnUserPlayBet(WORD wChairID, BYTE cbBetArea, LONGLONG lBetScore)
{
	...
	if (lBetScore<1000L || lBetScore>5000000L || (lBetScore%1000)!=0)
	{
		...
		_sntprintf(szMsg,CountArray(szMsg),TEXT("%s(%d)非法下注！"),pIUser->GetNickName(),pIUser->GetUserID());
...
bool CTableFrameSink::CalculateScore( OUT LONGLONG& lBankerWinScore, OUT tagServerGameRecord& GameRecord )
{
	...
	if (m_StorageControl.Judge(lSystemScore))
	{
		if (m_StorageControl.GetControled()==false)
		{
			for (int i=0;i<AREA_MAX;i++)
			{
				...
				{
					CTraceService::TraceString(TEXT("触发区域几率控制"),TraceLevel_Info);
					return false;
				}
			} 
		}
 ...
 //       if(lSystemScore >= 0L)
	...
	//		CString cs;
	//		cs.Format(TEXT("游戏结束,系统得分:%I64d,当前库存:%I64d"),lSystemScore,m_StorageStart);
	...
	//	{
	//		CTraceService::TraceString(TEXT("触发系统必赢"),TraceLevel_Debug);
	//		return false;
	//	}
