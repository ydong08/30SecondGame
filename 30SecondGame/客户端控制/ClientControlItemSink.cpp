...
void CClientControlItemSinkDlg::OnBnClickedResetStorage()
{
	...
	{
		//SetDlgItemText(IDC_STATIC_INFO,TEXT("操作失败，衰减值必须0到99"));
		AfxMessageBox(TEXT("操作失败，衰减值必须0到99之间"));
		return;
	}
...
{
	CString str;
	str.Format(TEXT("当前系统库存:%I64d   衰减值:%I64d"),pStorage->lStorage,pStorage->lStorageDeduct);
